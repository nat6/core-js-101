...
function willYouMarryMe(isPositiveAnswer) {
  ...
  return new Promise((resolve, reject) => {
    if (typeof isPositiveAnswer !== 'boolean') reject(new Error(err));
    else if (isPositiveAnswer) resolve(yes);
    else if (!isPositiveAnswer) resolve(no);
  });
}
...
function chainPromises(array, action) {
  return new Promise((res) => {
    ...
    array.forEach((promise) => {
      promise
        .then((currentRes) => results.push(currentRes))
        .catch((err) => new Error(`Wow! It's ${err} error`));
    });
